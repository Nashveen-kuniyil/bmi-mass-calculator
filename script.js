...
function calculateBMI(e) {
  ...
  if (age <= 30 && bodyMassIndex > 27.5) {
    document.querySelector('#bmiDescription').textContent = `Hey ${document.getElementById('user')} You are in the Obese range.`;
} 

else if (age <= 30 && bodyMassIndex >= 23 && bodyMassIndex <= 27.4) {
    document.querySelector('#bmiDescription').textContent ='You are in the Overweight range.';
}

 else if (age <= 30 && bodyMassIndex >= 19.5 && bodyMassIndex < 22.9) {
    document.querySelector('#bmiDescription').textContent ='You are in the Healthy Weight range.';
}

 else if (age <= 30 && bodyMassIndex < 19.5) {
    document.querySelector('#bmiDescription').textContent = 'You are in the Underweight range.';
}

 else if (age > 30 && bodyMassIndex > 28) {
    document.querySelector('#bmiDescription').textContent ='You are in the Obese range.';
}
  ...
  const speech=new SpeechSynthesisUtterance(document.querySelector('#bmiDescription').textContent);
